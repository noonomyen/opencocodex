    ...
    },
    {
      ...
      value: () => {
        ...
        if (created && completed && completed > created) {
          const toolDurationMs = ((sync().data.part[c.message.id] ?? []) as Part[]).reduce((sum, part) => {
          ...
          return `${((c.output + c.reasoning) / (Math.max(100, completed - created - toolDurationMs) / 1000)).toFixed(1)} tokens/s`
        }
        return "—"
      },
    },
